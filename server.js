// =====================================================
// SCOUT MULTIPLAYER — SERVER FINAL (roomUI 호환 + SHOW&SCOUT)
// =====================================================
...
io.on("connection", (socket) => {
  ...
  socket.on("startGame", ({ roomId, permUid }) => {
    const room = rooms[roomId];
    if (!room || permUid !== room.host) return;
  ...
  socket.on("startShowScout", ({ roomId, permUid }) => {
    ...
    io.to(p.socketId).emit("enterScoutMode", permUid);
  });
