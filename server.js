...
io.on("connection", (socket) => {
  ...
  socket.on("show", ({ roomId, permUid, cards }) => {
    ...
    // turn check
    const currentUid = room.turnOrder?.[room.currentTurn];
    if (currentUid && currentUid !== permUid) {
      io.to(room.players?.[permUid]?.socketId).emit("actionError", "지금은 당신의 턴이 아니에요.");
      return;
    }
    ...
    if (type === "invalid") {
      io.to(player.socketId).emit("actionError", "유효한 조합(RUN/SET)만 SHOW할 수 있어요.");
      return;
    }

    // Validate strength
    if (!isStrongerCombo(cards, room.table)) {
      io.to(player.socketId).emit("actionError", "테이블보다 강한 조합만 SHOW할 수 있어요.");
      return;
    }

    // Remove chosen cards from hand (allow reversed orientation)
    const chosen = cards.slice();
    const newHand = [...player.hand];
    for (const c of chosen) {
      const idx = newHand.findIndex(
        (h) => (h.top === c.top && h.bottom === c.bottom) || (h.top === c.bottom && h.bottom === c.top)
      );
      if (idx === -1) {
        io.to(player.socketId).emit("actionError", "패에 없는 카드를 SHOW하려고 했어요.");
        return;
      }
    ...
    // Scoring (closer to Scout token flow): successful SHOW gives +1
    player.score += 1;
