...
function renderHand() {
  ...
  if (insertMode) {
    ...
    requestAnimationFrame(() => {
      ...
      const handRect = cardsLayer.getBoundingClientRect();
      const baseRect = handArea.getBoundingClientRect();
      const rects = wraps.map(w => w.getBoundingClientRect());

      btns.forEach((btn, pos) => {
        // left is the CENTER x because the button uses translate(-50%, -50%)
        const bw = btn.getBoundingClientRect().width || 32;
        const bh = btn.getBoundingClientRect().height || 32;

        let x;
        if (pos === 0) {
          // slightly outside the first card
          x = (rects[0].left - baseRect.left) - (bw * 0.15);
        } else if (pos === rects.length) {
          // slightly outside the last card
          x = (rects[rects.length - 1].right - baseRect.left) + (bw * 0.15);
        } else {
          x = (rects[pos - 1].right + rects[pos].left) / 2 - baseRect.left;
        }

        // Allow a little overflow (looks natural at the edges) but avoid extreme values.
        const minX = -bw * 0.25;
        const maxX = baseRect.width + bw * 0.25;
        x = Math.max(minX, Math.min(maxX, x));
        btn.style.left = `${x}px`;

        // Place the button in the LOWER gap area between cards (user preference).
        // We position it slightly ABOVE the card bottoms so it's easy to tap.
        let bottom;
        if (pos === 0) {
          ...
          bottom = Math.max(rects[pos - 1].bottom, rects[pos].bottom);
        }

        let y = (bottom - baseRect.top) - Math.max(18, bh * 0.65);
        // Keep it within the handArea (allow some overflow; handBoard is visible overflow).
