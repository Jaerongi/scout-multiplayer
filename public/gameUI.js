  ...
  if (insertMode) {
    ...
    requestAnimationFrame(() => {
      ...
      btns.forEach((btn, pos) => {
        ...
        let x;
        if (pos === 0) {
          x = rects[0].left - baseRect.left;
        } else if (pos === rects.length) {
          x = rects[rects.length - 1].right - baseRect.left;
        } else {
        ...
        const min = bw * 0.6;
        const max = baseRect.width - bw * 0.6;
        x = Math.max(min, Math.min(max, x));

        btn.style.left = `${x}px`;

        // Place the button on top of the cards (not below), aligned near the bottom edge.
        const bh = btn.getBoundingClientRect().height || 32;
        const anchorRect = rects[Math.min(pos, rects.length - 1)];
        const y = (anchorRect.bottom - baseRect.top) - (bh * 0.55);
