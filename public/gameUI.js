  ...
  if (insertMode) {
    ...
    requestAnimationFrame(() => {
      ...
      const placeButton = (btn, pos) => {
        ...
        // Y: near the LOWER gap line between cards (your red-circle line)
        let bottom;
        if (pos === 0) bottom = rects[0].bottom;
        else if (pos === n) bottom = rects[n - 1].bottom;
        else bottom = Math.max(rects[pos - 1].bottom, rects[pos].bottom);

        let y = (bottom - baseRect.top) - Math.max(18, bh * 0.65);
        y = Math.max(bh * 0.55, Math.min(baseRect.height - bh * 0.35, y));
        btn.style.top = `${y}px`;
