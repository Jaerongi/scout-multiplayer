...
function applyHandSizing(cardCount) {
  ...
  // Clamp to a reasonable range (slightly smaller on phones) AND
  // keep the hand visible vertically (fan + action bar on mobile).
  const maxWByDevice = window.innerWidth <= 420 ? 102 : 124;
  const maxHByViewport = Math.floor(window.innerHeight * (window.innerWidth <= 420 ? 0.26 : 0.30));
  const maxWByHeight = Math.floor(maxHByViewport / 1.45);

  const maxW = Math.max(60, Math.min(maxWByDevice, maxWByHeight));
  const cardW = Math.max(48, Math.min(maxW, Math.floor(rawCardW)));
...
function renderHand() {
  handArea.innerHTML = "";
  // Cards layer keeps all cards in one stacking context so overlay buttons can sit above.
  const cardsLayer = document.createElement("div");
  cardsLayer.className = "handCards";
  handArea.appendChild(cardsLayer);
  ...
  disp.forEach((c, i) => {
    ...
    // Fan rotation (keep even in insert mode so the hand shape stays identical)
    {
  ...
  if (insertMode) {
    ...
    // We'll measure real card positions first, then create EXACTLY (n + 1) buttons (ends + between cards).
    requestAnimationFrame(() => {
      const wraps = Array.from(cardsLayer.querySelectorAll('.card-wrapper'));
      if (wraps.length === 0) return;

      const baseRect = handArea.getBoundingClientRect();
      const rects = wraps.map(w => w.getBoundingClientRect());
      const n = rects.length;

      // Build overlay contents fresh
      overlay.innerHTML = '';
      ...
      const clearHighlights = () => {
        wraps.forEach(w => w.classList.remove('insert-near'));
        marker.style.opacity = '0';
      };

      const placeButton = (btn, pos) => {
        const bw = btn.getBoundingClientRect().width || 32;
        const bh = btn.getBoundingClientRect().height || 32;

        // X: center of the gap
        let x;
        if (pos === 0) {
          x = (rects[0].left - baseRect.left) - (bw * 0.15);
        } else if (pos === n) {
          x = (rects[n - 1].right - baseRect.left) + (bw * 0.15);
        } else {
          x = (rects[pos - 1].right + rects[pos].left) / 2 - baseRect.left;
        }

        const minX = -bw * 0.25;
        const maxX = baseRect.width + bw * 0.25;
        x = Math.max(minX, Math.min(maxX, x));
        btn.style.left = `${x}px`;

        // Y: place the button around the "middle gap" line of the cards (not at the very bottom).
        // We anchor to ~62% of card height from the top (roughly where the wave boundary sits),
        // then average adjacent cards for between-card gaps.
        const pickYOnCard = (r) => r.top + (r.height * 0.62);
        let yAbs;
        if (pos === 0) yAbs = pickYOnCard(rects[0]);
        else if (pos === n) yAbs = pickYOnCard(rects[n - 1]);
        else yAbs = (pickYOnCard(rects[pos - 1]) + pickYOnCard(rects[pos])) / 2;

        let y = (yAbs - baseRect.top);
        // Keep within board (centered by translate(-50%,-50%))
        y = Math.max(bh * 0.55, Math.min(baseRect.height - bh * 0.55, y));
        btn.style.top = `${y}px`;

        return { x, y };
      };

      // Create buttons exactly n+1
      const btns = [];
      for (let pos = 0; pos <= n; pos++) {
        const btn = createInsertButton(pos, 0);
      ...
      // Position buttons
      btns.forEach((btn, pos) => placeButton(btn, pos));

      // Hover/focus highlight
      btns.forEach((btn, pos) => {
        const onEnter = () => {
          clearHighlights();
          if (pos === 0) wraps[0]?.classList.add('insert-near');
          else if (pos === n) wraps[n - 1]?.classList.add('insert-near');
          else {
            wraps[pos - 1]?.classList.add('insert-near');
            wraps[pos]?.classList.add('insert-near');
          }
          const { x } = placeButton(btn, pos);
          marker.style.left = `${x}px`;
          marker.style.opacity = '1';
        };

        const onLeave = () => clearHighlights();
