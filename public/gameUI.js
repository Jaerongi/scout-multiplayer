...
function applyHandSizing(cardCount) {
  ...
  const cardW = Math.max(54, Math.min(122, Math.floor(rawCardW)));
...
function renderHand() {
  ...
  // Fan/overlap only in normal play (not in insert mode)
  handArea.classList.toggle('fan', !insertMode);
...
window.addEventListener('resize', () => {
  try { applyHandSizing(getDisplayedHand().length); } catch {}
});
