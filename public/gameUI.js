  ...
  if (insertMode) {
    ...
    requestAnimationFrame(() => {
      ...
      btns.forEach((btn, pos) => {
        ...
        // Place the button near the TOP gap area (like the official UI), not at the bottom.
        let top;
        if (pos === 0) {
          top = rects[0].top;
        } else if (pos === rects.length) {
          ...
          top = Math.min(rects[pos - 1].top, rects[pos].top);
        }

        // Offset a bit downward so it sits on the card face.
        let y = (top - baseRect.top) + Math.max(14, bh * 0.35);
        // Keep it within the handArea.
