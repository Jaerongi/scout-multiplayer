...
function renderHand() {
  ...
  if (insertMode) {
    ...
    // Read sizing vars from CSS
    const cs = getComputedStyle(handArea);
    const cardW = parseFloat(cs.getPropertyValue("--card-w")) || 92;
    const overlapPx = parseFloat(cs.getPropertyValue("--overlap-px")) || 58;
    const shift = Math.max(6, cardW - overlapPx);

    // Position buttons at each gap boundary without changing layout
    // pos 0..n
    for (let pos = 0; pos <= disp.length; pos++) {
      const x = pos * shift;
      overlay.appendChild(createInsertButton(pos, x));
    }
  }
}
